...
export class UserService {
  ...
  constructor(private prisma: PrismaService, @Inject(forwardRef(() => WalletService)) private walletService: WalletService) {
  ...
  async create(dto: UserCreateDto): Promise<User> {
    ...
    if((dto.seed) && dto.seed.trim().length > 0) {
      wallet = await this.walletService.generateWalletWithSeed(user.id,dto.seed);
    }
    else{
      wallet = await this.walletService.generateWallet(user.id);
    }
